...
import typeOfDocumentResolve from './type-of-document-routing-resolve.service';

describe('TypeOfDocument routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ITypeOfDocument returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          typeOfDocumentResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(typeOfDocumentResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
