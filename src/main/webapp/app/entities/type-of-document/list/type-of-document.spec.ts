...
import { sampleWithRequiredData } from '../type-of-document.test-samples';

import { TypeOfDocument } from './type-of-document';

vitest.useFakeTimers();

describe('TypeOfDocument Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TypeOfDocument;
  let fixture: ComponentFixture<TypeOfDocument>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TypeOfDocument);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 8066 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.typeOfDocuments()[0]).toEqual(expect.objectContaining({ id: 8066 }));
  });

  describe('trackId', () => {
    it('should forward to typeOfDocumentService', () => {
      const entity = { id: 8066 };
      vitest.spyOn(service, 'getTypeOfDocumentIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTypeOfDocumentIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.typeOfDocumentsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
