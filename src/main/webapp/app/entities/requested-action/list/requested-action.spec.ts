...
import { sampleWithRequiredData } from '../requested-action.test-samples';
...
describe('RequestedAction Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: RequestedAction;
  let fixture: ComponentFixture<RequestedAction>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(RequestedAction);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 13303 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.requestedActions()[0]).toEqual(expect.objectContaining({ id: 13303 }));
  });

  describe('trackId', () => {
    it('should forward to requestedActionService', () => {
      const entity = { id: 13303 };
      vitest.spyOn(service, 'getRequestedActionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRequestedActionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.requestedActionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
