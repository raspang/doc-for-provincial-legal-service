...
import { sampleWithRequiredData } from '../document-status.test-samples';
...
describe('DocumentStatus Management Component', () => {
  ...
  let comp: DocumentStatus;
  let fixture: ComponentFixture<DocumentStatus>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DocumentStatus);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 22980 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.documentStatuses()[0]).toEqual(expect.objectContaining({ id: 22980 }));
  });

  describe('trackId', () => {
    it('should forward to documentStatusService', () => {
      const entity = { id: 22980 };
      vitest.spyOn(service, 'getDocumentStatusIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDocumentStatusIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.documentStatusesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
