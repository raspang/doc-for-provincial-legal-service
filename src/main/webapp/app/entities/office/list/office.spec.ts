...
import { sampleWithRequiredData } from '../office.test-samples';
...
describe('Office Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20465 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.offices()[0]).toEqual(expect.objectContaining({ id: 20465 }));
  });

  describe('trackId', () => {
    it('should forward to officeService', () => {
      const entity = { id: 20465 };
      vitest.spyOn(service, 'getOfficeIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.officesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
