...
import { sampleWithRequiredData } from '../document-reference.test-samples';
...
describe('DocumentReference Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: DocumentReference;
  let fixture: ComponentFixture<DocumentReference>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DocumentReference);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29885 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.documentReferences()[0]).toEqual(expect.objectContaining({ id: 29885 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 29885 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.documentReferences()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 29885 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to documentReferenceService', () => {
      const entity = { id: 29885 };
      vitest.spyOn(service, 'getDocumentReferenceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDocumentReferenceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.documentReferencesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.documentReferencesParams()).toMatchObject(
