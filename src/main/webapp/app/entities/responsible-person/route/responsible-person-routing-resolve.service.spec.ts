...
import responsiblePersonResolve from './responsible-person-routing-resolve.service';
...
describe('ResponsiblePerson routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IResponsiblePerson returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          responsiblePersonResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(responsiblePersonResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
