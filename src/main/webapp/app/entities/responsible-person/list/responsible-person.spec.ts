...
import { sampleWithRequiredData } from '../responsible-person.test-samples';
...
describe('ResponsiblePerson Management Component', () => {
  ...
  let comp: ResponsiblePerson;
  let fixture: ComponentFixture<ResponsiblePerson>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ResponsiblePerson);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29109 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.responsiblePeople()[0]).toEqual(expect.objectContaining({ id: 29109 }));
  });

  describe('trackId', () => {
    it('should forward to responsiblePersonService', () => {
      const entity = { id: 29109 };
      vitest.spyOn(service, 'getResponsiblePersonIdentifier');
      const id = comp.trackId(entity);
      expect(service.getResponsiblePersonIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.responsiblePeopleParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
