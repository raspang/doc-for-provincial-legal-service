...
import { sampleWithRequiredData } from '../transaction-type.test-samples';
...
describe('TransactionType Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4045 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.transactionTypes()[0]).toEqual(expect.objectContaining({ id: 4045 }));
  });

  describe('trackId', () => {
    it('should forward to transactionTypeService', () => {
      const entity = { id: 4045 };
      vitest.spyOn(service, 'getTransactionTypeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTransactionTypeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.transactionTypesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should infinite scroll', async () => {
    ...
    req.flush([{ id: 4045 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
    expect(comp.transactionTypes()).toHaveLength(1);
    expect(comp.transactionTypes()[0]).toEqual(expect.objectContaining({ id: 4045 }));

    // WHEN
    comp.loadNextPage();
    TestBed.tick();
    expect(service.transactionTypesParams()).toMatchObject(expect.objectContaining({ page: '1' }));
    ...
    await vitest.runAllTimersAsync();
    expect(comp.transactionTypes()).toHaveLength(2);
    expect(comp.transactionTypes()[1]).toEqual(expect.objectContaining({ id: 32159 }));

    comp.loadNextPage();
    TestBed.tick();
    expect(service.transactionTypesParams()).toMatchObject(expect.objectContaining({ page: '2' }));
    req = httpMock.expectOne({ method: 'GET' });
    req.flush([{ id: 32159 }], {
      headers: { link: '<http://localhost/api/foo?page=0&size=20>; rel="prev",<http://localhost/api/foo?page=2&size=20>; rel="next"' },
    });
